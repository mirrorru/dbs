...
//nolint:gocognit
func getFieldInfo0(t reflect.Type) (resultList FieldInfoList, err error) {
	...
	for i := range t.NumField() {
		...
		if field.Anonymous && field.Type.Kind() != reflect.Ptr {
			// Can't refer to fields of reference
		...
		// Inline tag-directive
		if fieldCfg.isInline && field.Type.Kind() != reflect.Ptr {
			if info, err = peekStructInfo(field.Type); err != nil {
				return nil, err
			}
			for _, fld := range info.allFields {
